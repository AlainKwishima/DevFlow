...
export async function getSavedQuestions(params: GetSavedQuestionsParams) {
  try {
    ...
    let user = await User.findOne({ clerkId }).populate({
      path: "saved",
      match: query,
      options: {
        sort: sortOptions,
    ...
    if (!user) {
      ...
      // This can happen if the webhook didn't fire or there was an issue
      console.log(`User with clerkId ${clerkId} not found, creating basic user record`);
      user = await User.create({
        clerkId,
        name: "User",
        username: `user_${clerkId.slice(-6)}`,
        email: `user_${clerkId.slice(-6)}@example.com`,
        picture: "https://img.clerk.com/preview.png",
        reputation: 0,
        saved: [],
        joinedAt: new Date(),
      });
...
export async function getUserInfo(params: GetUserByIdParams) {
  try {
    ...
    if (!user) {
      // If user doesn't exist, create a basic user record
      console.log(`User with clerkId ${userId} not found, creating basic user record`);
      user = await User.create({
        clerkId: userId,
        name: "User",
